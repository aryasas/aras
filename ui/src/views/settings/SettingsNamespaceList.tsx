...
import { Activity, Database, History, Key, LayoutDashboard, Package, Server, Settings, Shield, Terminal, UploadCloud } from 'lucide-react'
...
type Shortcut = {
  ...
  group: 'preferences' | 'access' | 'platform' | 'tools'
  adminOnly?: boolean
...
const SHORTCUTS: Shortcut[] = [
  { to: '/admin/settings/dashboard', label: 'Dashboard Builder', sub: 'admin/settings/dashboard', icon: LayoutDashboard, group: 'preferences', adminOnly: true },
  { to: '/admin/settings/access', label: 'Access Control', sub: 'admin/settings/access', icon: Shield, group: 'access' },
  ...
  { to: '/control-panel/plans', label: 'SaaS Plans', sub: 'control-panel/plans', icon: Server, group: 'platform', adminOnly: true, controlPanelOnly: true },
  ...
  { to: '/admin/settings/files', label: 'File Manager', sub: 'admin/settings/files', icon: UploadCloud, group: 'tools' },
  { to: '/admin/settings/audit', label: 'Activity Audit', sub: 'admin/settings/audit', icon: History, group: 'tools' },
]
...
function ShortcutLink({ shortcut, active }: { shortcut: Shortcut; active: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        <span className="block truncate text-[13px] font-semibold">{shortcut.label}</span>
        <span className="arc-mono block truncate text-[10px] uppercase tracking-[0.12em] text-[var(--text-3)]">{shortcut.sub}</span>
      </span>
    </Link>
  )
}
...
export default function SettingsNamespaceList({ selectedNamespace, onSelect, onLoaded }: SettingsNamespaceListProps) {
  ...
  const visibleShortcuts = useMemo(() => SHORTCUTS.filter((shortcut) => {
    if (shortcut.adminOnly && !user?.is_admin) return false
    if (shortcut.controlPanelOnly && arasRole !== 'control-panel') return false
    if (shortcut.tenantOnly && arasRole === 'control-panel') return false
    return true
  }), [arasRole, user?.is_admin])

  const groupedShortcuts = useMemo(() => (
    (['preferences', 'access', 'platform', 'tools'] as const)
      .map((group) => ({ group, items: visibleShortcuts.filter((shortcut) => shortcut.group === group) }))
  ...
  return (
    <nav className="flex flex-col gap-1 p-3" aria-label="Settings namespaces">
      <div className="mb-2">
        <div className="arc-mono mb-1 px-3 pt-1 text-[10px] uppercase tracking-[0.14em] text-[var(--text-3)]">{GROUP_LABELS.general}</div>
        {namespaces.length === 0 ? (
          <div className="px-3 py-2 text-[12px] text-[var(--text-3)]">No app settings are available.</div>
        ) : namespaces.map((namespace) => {
          const Icon = namespace.icon ? resolveIcon(namespace.icon) : Settings
          const active = selectedNamespace === namespace.name && location.pathname === '/admin/settings'
          return (
            <button
              key={namespace.name}
              type="button"
        ...
        <div key={group} className="mb-2">
          ...
          {items.map((shortcut) => (
            <ShortcutLink
              key={shortcut.to}
              shortcut={shortcut}
              active={location.pathname === shortcut.to}
            />
          ))}
        </div>
      ))}
