...
export const GenericReport: React.FC<GenericReportProps> = ({ title, data, columns, onBack }) => {
  ...
  const handleExportCSV = () => {
    ...
    link.download = `${title.toLowerCase().replace(/\s+/g, '_')}_report.csv`;
    link.click();
  };
  ...
  return (
    <div className="bg-white min-h-screen p-8 print:p-0">
      ...
      <div className="flex items-center justify-between mb-8 print:hidden">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all"
          >
            <Download size={18} />
          ...
          <button 
            onClick={handlePrint}
            className="flex items-center gap-2 px-6 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100"
          >
            <Printer size={18} />
      ...
      <div className="space-y-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 print:bg-white">
                {columns.map(col => (
                  <th key={col.field} className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-widest">
                    {col.label}
                  </th>
                ))}
              </tr>
              ...
              {data.map((row, idx) => (
                <tr key={idx} className="border-b border-slate-100 last:border-none hover:bg-slate-50/50 transition-colors">
                  {columns.map((col, colIdx) => {
                    // Support both object rows {field: val} and array rows [val, val]
                    const val = Array.isArray(row) ? row[colIdx] : row[col.field];
                    return (
                      <td key={col.field} className="px-6 py-4 text-sm text-slate-700 font-medium">
                        {col.type === 'currency' ? (
        ...
        <div className="hidden print:block pt-12 text-[10px] text-slate-400 text-center border-t border-slate-100 mt-20">
