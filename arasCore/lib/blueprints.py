...
def _register_helper(flask_app, helper):
    ...
    for cr in helper.custom_routes:
        ...
        ep_name  = f"api_{helper.name}_{clean}"
    ...
    # ── 4. Auto-mount /admin/<app>/ home page ─────────────────────────────────
    try:
    ...
    # ── 5. Auto-mount /admin/<app>/<slug>/ for MenuGroup sub-pages ──────
...
def _register_aras_apps(app):
    ...
    for entry in sorted(os.listdir(aras_pkg)):
        ...
        try:
            ...
            # Load manifest regardless of whether views has app_bp.
            # Apps that use AppHelper don't need a Blueprint in views.py.
            ...
            if helper:
                ...
                logger.warning(f"[blueprints] {pkg_name}.views has no app_bp and no manifest — skip")
            else:
