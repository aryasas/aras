...
def _register_helper(flask_app, helper):
    ...
    """
    from arasCore.lib.services.api_handler import register_api_model, register_custom_route
    ...
    try:
        from arasCore.admin.home_service import mount_home_route
        mount_home_route(
            helper_bp, helper.admin_slug, helper.title,
            is_dynamic=False,
    ...
    if helper.menu_groups:
        try:
            from arasCore.admin.home_service import mount_group_route
            mount_group_route(
                helper_bp, helper.admin_slug, helper.title,
...
def _mount_admin_resource(bp, res, adm_prefix, helper):
    ...
    from arasCore.lib.ui.admin_mount import AdminResourceMounter
    AdminResourceMounter(bp, res, adm_prefix, helper).mount()


def _is_app_enabled(entry: str, aras_pkg: str) -> bool:
    """
    Return True if the app package should be loaded.
    ...
    # Derivce slug: strip "app_" prefix if it exists, otherwise use entry directly
    app_name = entry[len("app_"):] if entry.startswith("app_") else entry
    try:
...
def _register_aras_apps(app):
    ...
    # List all subdirectories in aras/ package
    for entry in sorted(os.listdir(aras_pkg)):
        full_path = os.path.join(aras_pkg, entry)
        if not os.path.isdir(full_path) or entry.startswith(".") or entry.startswith("__") or entry in _SKIP:
            continue

        has_views = (
            os.path.isfile(os.path.join(full_path, "views.py")) or
            os.path.isfile(os.path.join(full_path, "views", "__init__.py"))
        )
        has_manifest = os.path.isfile(os.path.join(full_path, "manifest.py"))
...
def register_app_modules(app):
    ...
    Load app_* dari aras/ lalu register admin terakhir.
    """
