...
from arasCore.lib.ui.label_utils import humanize as _humanize_label, row_display, find_ref_model as _find_ref_model
...
# ── menu_service ──────────────────────────────────────────────────────────────
from arasCore.admin.menu_service import (  # noqa: E402
    _build_raw_menu,
    build_sidebar_menu,
...
# ── crud_factory ──────────────────────────────────────────────────────────────
from arasCore.admin.crud_factory import (  # noqa: E402
    _invoke_hooks,
    _load_activity_log,
...
def _register_built_app(app_def_id, flask_app):
    ...
    try:
        ...
        try:
            from arasCore.admin.home_service import mount_home_route, mount_group_route
            mount_home_route(
                bp, admin_slug, app_main_title,
                is_dynamic=True,
            ...
            mount_group_route(
                bp, admin_slug, app_main_title,
                endpoint=f"group_{admin_slug}",
                registry_key=app_name,
            )
        except Exception as _he:
            logger.warning(f"[services] home/group mount failed for {admin_slug}: {_he}")
    ...
    for cs in child_snaps:
        ...
        if fk_col:
            ...
            child_defs.append({
                ...
                "model":          cs["model"],
                "vcols":          cs["vcols"],
                "all_columns":    _all_model_columns(cs["model"]),
                ...
                "price_type":     getattr(cs["model"], "__price_type__", "sales"),
                "model_name":     cs["model"].__tablename__,
                "app_id":         cs["app_id"],
