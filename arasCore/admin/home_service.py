# -*- coding: utf-8 -*-
"""
arasCore/admin/home_service.py
==================================
...
def _get_page_tiles(app_name: str, registry_key: str = None):
    ...
    for t in rows:
        ...
        if t.parent_table_id:
            parent = AppManagerTable.query.get(t.parent_table_id)
            if parent:
                group_name = parent.get_menu_title()
...
def make_group_view(app_name: str, app_title: str, registry_key: str = None):
    ...
    def view(group_slug: str):
        ...
        return render_template(
            ...
            group_icon=grp_rec.icon or grp_rec.menu_icon or "fa-folder",
            tiles=tiles,
