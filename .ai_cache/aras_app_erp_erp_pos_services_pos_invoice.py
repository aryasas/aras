...
def create_invoice_from_pos(order_id: int) -> AccSalesInvoice:
    ...
    for line in order.lines:
        ...
        if product and product.account_links:
            link = next((l for l in product.account_links if l.company_id == company_id), None)
            if link and link.account_revenue_id:
                revenue_acc_id = link.account_revenue_id
        elif product and product.category and product.category.account_revenue_id:
            revenue_acc_id = product.category.account_revenue_id
