...
from app.erp.erp_stock.models.delivery import DeliveryOrder, DeliveryOrderLine, DeliveryTrip
...
def create_delivery_from_invoice(invoice_id: int) -> DeliveryOrder:
    ...
    do_name = seq_svc.next_code(inv.company_id, "stock.delivery_order", default_format="DO-{YYYY}-{####}")
    do = DeliveryOrder(
...
def create_delivery_from_order(order_id: int) -> DeliveryOrder:
    ...
    from app.erp.erp_acc.models.sales_order import SalesOrder
    ...
    do_name = seq_svc.next_code(order.company_id, "stock.delivery_order", default_format="DO-{YYYY}-{####}")
    do = DeliveryOrder(
...
def mark_delivered(delivery_id: int, posted_by_id: int = None) -> DeliveryOrder:
    ...
    if do.src_location_id:
        ...
        mv_name    = seq_svc.next_code(company_id, "stock.movement", default_format="SM-{YYYY}-{####}")
