...
def get_price(product_id: int, uom_id: int, qty: Decimal,
    ...
    def _standalone_price(pid, uid, q, pt) -> Decimal:
        # No standalone (pricelist-less) rows in normalized model;
        # fall back to the first PriceType matching the kind.
        pp = (
            _base_q()
            .join(SPT, SPP.price_type_id == SPT.id)
            .filter(SPP.product_id == pid, SPT.price_type == pt)
    ...
    if pricelist_id:
        ...
        def _pricelist_lookup(pt_filter):
            ...
            if pt_filter:
                q = q.join(SPT, SPP.price_type_id == SPT.id).filter(SPT.price_type == pt_filter)
