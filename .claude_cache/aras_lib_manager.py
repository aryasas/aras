...
from aras.lib.db import db
from aras.lib.base import BaseClass
from aras.lib.utils import Utilities as ut
from aras.lib.forms import DataSearchForm
from aras.lib.uploads import upload_image_file
...
class APIManager(RoutesMixin, MethodView):
    ...
    def put(self, id):
        ...
        try:
            result = schema.load(json_data, instance=data, partial=True)
            db.session.commit()
            return {"message": "Data updated.", "result": schema.dump(result)}, 200
        except ValidationError as err:
            return {"errors": err.messages}, 422
        except SQLAlchemyError as ex:
            db.session.rollback()
