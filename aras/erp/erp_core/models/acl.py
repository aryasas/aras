...
class ErpRole(ArasModel):
    __tablename__ = "erp_role"

    company_id  = db.Column(db.Integer, db.ForeignKey("company.id"), nullable=True)  # NULL=global
...
class ErpPermission(ArasModel):
    __tablename__ = "erp_permission"
...
class ErpRolePermission(db.Model):
    __tablename__ = "erp_role_permission"
    __table_args__ = (
        db.PrimaryKeyConstraint("role_id", "permission_id"),
    )

    role_id       = db.Column(db.Integer, db.ForeignKey("erp_role.id"), nullable=False)
    permission_id = db.Column(db.Integer, db.ForeignKey("erp_permission.id"), nullable=False)

    permission = db.relationship("ErpPermission")


class ErpUserCompany(db.Model):
    __tablename__ = "erp_user_company"
    __table_args__ = (
    ...
    role_id    = db.Column(db.Integer, db.ForeignKey("erp_role.id"), nullable=True)
