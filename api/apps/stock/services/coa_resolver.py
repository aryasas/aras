...
def _get_Account():
    acc = ServiceRegistry.get("Account")
    if acc is None:
        raise RuntimeError("AccountingService 'Account' not registered; is the accounting app installed?")
    return acc


def _get_acc_default():
    fn = ServiceRegistry.get("acc_default")
    if fn is None:
        raise RuntimeError("AccountingService 'acc_default' not registered; is the accounting app installed?")
    return fn
...
class CoaResolver:
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_stock_account(db: Session, product_id: int, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_cogs_account(db: Session, product_id: int, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_revenue_account(db: Session, product_id: int, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_expense_account(db: Session, product_id: int, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_ar_account(db: Session, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_ap_account(db: Session, org_id: int) -> Optional[object]:
        Account = _get_Account()
        acc_default = _get_acc_default()
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_tax_account(db: Session, org_id: int, direction: str) -> Optional[object]:
        Account = _get_Account()

        effective_org_id = CoaResolver._get_effective_org_id(db, org_id)
        account_type = "liability_current" if direction == "payable" else "asset_current"
        return db.query(Account).filter(
            Account.org_id == effective_org_id,
    ...
    # claude-sonnet-4-6
    @staticmethod
    def resolve_variance_account(db: Session, product_id: int, org_id: int) -> Optional[object]:
        Account = _get_Account()
