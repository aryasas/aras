...
class WorkflowTemplate(Aras.Model):
    __tablename__ = "core_workflow_templates"
    __features__ = ["audit"]
...
class WorkflowState(Aras.Model):
    ...
    template_id: Mapped[int] = mapped_column(ForeignKey("core_workflow_templates.id"), index=True)
...
class WorkflowTransition(Aras.Model):
    ...
    template_id: Mapped[int] = mapped_column(ForeignKey("core_workflow_templates.id"), index=True)
    from_state_id: Mapped[int] = mapped_column(ForeignKey("core_workflow_states.id"))
    to_state_id: Mapped[int] = mapped_column(ForeignKey("core_workflow_states.id"))
    name: Mapped[str] = mapped_column(String(50))
    label: Mapped[str] = mapped_column(String(100))
...
class WorkflowAction(Aras.Model):
    ...
    transition_id: Mapped[int] = mapped_column(ForeignKey("core_workflow_transitions.id"), index=True)
