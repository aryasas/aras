...
async def install_app(
    ...
    try:
        if filename.endswith(".zip"):
            installer = AppInstaller.install_from_zip(content, db)
        elif filename.endswith(".yaml") or filename.endswith(".yml"):
            installer = AppInstaller.install_from_yaml(content.decode(), db)
        elif filename.endswith(".json"):
            installer = AppInstaller.install_from_json(content.decode(), db)
        else:
            raise HTTPException(status_code=400, detail="Unsupported file format")
            
        return {"status": "success", "message": f"App installed successfully"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.delete("/uninstall/{app_name}")
async def uninstall_app(app_name: str, db: Session = Depends(get_db)):
    """
