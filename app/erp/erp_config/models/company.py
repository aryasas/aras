...
class Company(ArasGen.Model, module=Config):
    __title__     = "Company"
    __icon__      = "fa-building-o"
    __menu_order__= 0
    __tablename__ = "cfg_company"
    ...
    base_currency_id        = db.Column(db.Integer, db.ForeignKey("cfg_currency.id"), nullable=True)
    fiscal_year_start_month = db.Column(db.SmallInteger, default=1)
    default_charge_id       = db.Column(db.Integer, db.ForeignKey("cfg_charge.id"), nullable=True)
    ...
    parent_id               = db.Column(db.Integer, db.ForeignKey("cfg_company.id"), nullable=True)
    ...
    # Tax accounts
    acc_tax_output_ppn_id            = db.Column(db.BigInteger, db.ForeignKey("acc_account.id"), nullable=True)
    acc_tax_input_ppn_id             = db.Column(db.BigInteger, db.ForeignKey("acc_account.id"), nullable=True)
