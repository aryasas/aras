...
class Currency(ArasGen.Model, module=Config):
    __title__     = "Currency"
    __icon__      = "fa-dollar"
    __menu_order__= 0
    __tablename__ = "cfg_currency"
...
class FxRate(ArasGen.Model, module=Config):
    __is_child__  = True
    __tablename__ = "cfg_fx_rate"
    __table_args__ = (
    ...
    from_currency_id = db.Column(db.Integer, db.ForeignKey("cfg_currency.id"), nullable=False)
    to_currency_id   = db.Column(db.Integer, db.ForeignKey("cfg_currency.id"), nullable=False)
